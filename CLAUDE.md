...
## Vault Contents
**Read [`index.md`](index.md) first** to find what's in this vault. It's the curated navigation hub — every section, subdirectory, and note has a one-line description and wikilink. Use it to locate relevant notes before starting a task instead of grepping blindly. Regenerate it via [`meta\updateIndex.md`](updateIndex.md) when the vault structure changes.
