...
Scikit-learn compatible transformers for spectral and EIS data. All inherit `BaseEstimator` / `TransformerMixin`. Input/output are pandas DataFrames unless noted.
...
- Drops near-constant columns that carry no signal
- `featureMask` computed during `fit()`, applied in `transform()`

### `IdentityTransformer()`
Pass-through transformer that returns data unchanged. Declares metadata routing (`meta="ignore"`) for consistent use in pipelines alongside other transformers.
- Used for identity/raw pipelines with no processing steps
...
### `ExtractFeaturesEIS()`
Applies `prcCplxEIS` row-wise. No parameters.
Outputs 31 EIS features per row — see `prcPandas.prcCplxEIS` for feature list.
...
Each dict has:
- `"name"` (str, required): transformer name from the registry
